...
public class SolidColor extends AbstractConfigurationObject implements Color {
    ...
    public SolidColor(final String color) {
        this.color = color.replaceAll("[^0-9a-z,A-Z#]", "");
    }

    public SolidColor(final int red, final int green, final int blue) {
        if (red < 0 || red > 255 || green < 0 || green > 255 || blue < 0 || blue > 255) {
            throw new IllegalArgumentException("Invalid color values given.");
        }
        this.color = String.format(Locale.ENGLISH, "#%02X%02X%02X", red, green, blue);
    }

    public SolidColor(final int red, final int green, final int blue, final double opacity) {
        if (red < 0 || red > 255 || green < 0 || green > 255 || blue < 0 || blue > 255 || opacity < 0.0 || opacity > 1.0) {
            throw new IllegalArgumentException(String.format("Invalid color values given. Red: %s, green: %s blue: %s, opacity: %s", red, green, blue, opacity));
        }
        this.color = String.format(Locale.ENGLISH, "rgba(%d,%d,%d,%.2f)", red, green, blue, opacity);
    }
