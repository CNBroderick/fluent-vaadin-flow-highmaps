...
import './chart-deep-merger.js';

class MapElement extends ElementMixin(ThemableMixin(PolymerElement)) {
    ...
    static get template() {
        return html`
            ...
            <div id="map">default content</div>
        `;
    }

    static get is() {
        return 'high-charts-map'
    }
    ...
    static get properties() {
        ...
        property._mapData = this._chinaMapData();
        return property;
    }
    ...
    __makeConfigurationBuffer(target, source) {
        ...
        if (source.chart.map) {
            source.chart.map = this.getMapData();
        }
        if (source.series && Array.isArray(source.series)) {
            source.series.forEach(series => {
                if (series.mapData) series.mapData = this.getMapData();
            });
        }
    ...
    __initMap(options) {
        ...
        this.configuration = Highcharts.mapChart(this.$.map, options);
    }
        ...
        for (let keys = Object.keys(eventList), i = 0; i < keys.length; i++) {
            ...
            if (!eventObject[key]) {
                ...
                eventObject[key] = function (event) {
                    ...
                    if (customEvent.detail.originalEvent.type == 'drilldown') {
                        // point is a huge object graph with a lot nested objects and arrays,
                        // so we don't pass it to java on drilldown event (only pointId)
                        // but put in component cache until '__addSeriesAsDrilldown' will be fired
                        // and then use this point in Chart#addSeriesAsDrilldown
                        if (!self.drilldownPoints) self.drilldownPoints = {};
                        const point = customEvent.detail.originalEvent.point;
                        self.drilldownPoints[point.id] = customEvent.detail.originalEvent.point;
                    }
        ...
        if (functionToCall && typeof functionToCall === 'function') {
            ...
            if (redrawCharts) {
                Highcharts.charts.forEach((c) => {
                    if (functionName == "setOptions") {
                        c.update(argumentsForCall[0], redrawCharts);
                    } else {
                        c.redraw();
                    }
                });
    ...
    __addSeriesAsDrilldown() {
        const argumentsForCall = Array.prototype.splice.call(arguments, 0);
        const point = this.drilldownPoints[argumentsForCall[0]];
        // clean up cache, which we use to keep point in component without passing it to java
        delete this.drilldownPoints[argumentsForCall[0]];
        const series = argumentsForCall[1];
        if (series.mapData) series.mapData = window.VaadinMaps.maps[series.mapData];
        this.configuration.addSeriesAsDrilldown(point, series);
    }
    ...
    __updateStyles() {
        ...
        if (isFlex) {
            this.$.map.setAttribute('style', 'flex: 1; -webkit-flex: 1; -ms-flex: 1; ');
            let style = '';
            ...
            this.$.map.setAttribute('style', 'height:100%; width:100%;');
        }
    }

    getMapData() {
        return this._mapData === null || this._mapData === undefined ? MapElement._chinaMapData() : this._mapData;
    }

    static _chinaMapData() {
        return {
            "title": "中国",
            "version": "0.0.3",
            ...
            "features": [{
                ...
                "geometry": {"type": "MultiPolygon", "coordinates": [[[[6234, 2355], [6243, 2337], [6233, 2331], [6234, 2355]]]]},
                "id": "CN.820000"
            }, {
                "type": "Feature",
                "id": "CN.000000",
